...
function resetmail($email,$reset_token)
{
    ...
    try {
        ...
        $mail->Body    = "We got a request from you to reset your password <br> Click the link below:<br>
          <a href='https://scanspree.shop/Scanspree/Admin/updatepassword.php?email=$email&reset_token=$reset_token'>Reset password</a>";
...
{
    $sql = "SELECT * FROM  `admin_login`WHERE `email`='$_POST[email]'";
    ...
    if ($result == true)
    {
        ...
        if ($result->num_rows == 1)
        {
           ...
           $date = date("y-m-d");
           $upquery = "UPDATE  `admin_login` SET `resettoken`='$reset_token', `resettokenexpire`='$date' where `email`='$_POST[email]'";
